diff --git a/routers.ts b/routers.ts
index 5b2fabd7e4a2c642a6757d5475e355b53a47cec0..e8c54009deddadac71bc9a908b7cbfdb75902906 100644
--- a/routers.ts
+++ b/routers.ts
@@ -1,42 +1,97 @@
 ...
 const buscaGlobalProcedure = protectedProcedure
   ...
   .query(async ({ input }) => {
     ...
     const pacientesResult = await drizzleDb
       ...
       .from(pacientes)
       .where(
@@ -206,53 +261,119 @@ export const appRouter = router({
...
@@ -307,50 +428,189 @@ export const appRouter = router({
         })
       )
       .mutation(async ({ input }) => {
         const consulta = await db.getConsultaById(input.consultaId);
...
+
     ...
     atualizarHipotesesCondutaIA: protectedProcedure
       .input(z.object({ consultaId: z.number() }))
       .mutation(async ({ input }) => {
@@ -648,74 +908,76 @@ REGRAS ADICIONAIS:
                         "guidelineReferencia",
                       ],
...
+            assinaturaTipo: input.assinatura,
     ...
     gerarDocumento: protectedProcedure
       ...
       .mutation(async ({ input }) => {
         ...
         if (!consulta) {
           throw new Error("Consulta não encontrada");
@@ -864,103 +1126,348 @@ REGRAS ADICIONAIS:
             resumo: null,
           };
         }
...
+
   ...
   planos: router({
     ...
     create: protectedProcedure
@@ -1031,69 +1538,70 @@ REGRAS ADICIONAIS:
     ...
     processarAnamnese: protectedProcedure
       ...
       .mutation(async ({ ctx, input }) => {
         try {
           ...
           // 2. Upload para S3
@@ -1101,94 +1609,174 @@ REGRAS ADICIONAIS:
...
@@ -1328,128 +1916,161 @@ REGRAS ADICIONAIS:
